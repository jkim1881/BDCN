...
class BDCN_ti(nn.Module):
    ...
    def center_crop(self, x, keepdims=True):
        ...
        if keepdims:
            crop = crop.unsqueeze(2).unsqueeze(2)
        return crop
    ...
    def initialize_ti_weights(self):
        # TI layers
        self.upsample_ti_2 = nn.Upsample(scale_factor=2, mode='bilinear')
        self.upsample_ti_4 = nn.Upsample(scale_factor=4, mode='bilinear')
